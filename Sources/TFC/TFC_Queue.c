...
for (i=0;i<BQ->QueueSize;i++) BQ->QueueStorage[i] = 0;
}
...
if (BytesInQueue(BQ) == BQ->QueueSize) 
    ...
    BQ->WritePtr++;
    if (BQ->WritePtr >= BQ->QueueSize) BQ->WritePtr = 0;        
    return QUEUE_OK;
    }
}
...
{
uint16_t i;
for (i=0;i<Len;i++) ByteEnqueue(BQ,Buf[i]);
return QUEUE_OK;
}
...
if (BytesInQueue(BQ) == 0) 
	...
	*Val  = BQ->QueueStorage[BQ->ReadPtr];
	BQ->ReadPtr++;
	if (BQ->ReadPtr >= BQ->QueueSize) BQ->ReadPtr = 0;	
	return QUEUE_OK;
	}
}
